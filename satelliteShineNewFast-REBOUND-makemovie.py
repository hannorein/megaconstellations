...
def getFig(hour):
    ...
    timesOfYear = {"December solstice":-np.pi/2.}
    magVmin, magVmax =5, 8
    ...
    for i, latitudename in enumerate(latitudes):
        ...
        for j, timeOfYear in enumerate(timesOfYear):
            ...
            ax.text(-1.11,-1.1,"N$_{vis}$=%d"%len(magV[magV<6.5]))
...
#hours = np.linspace(-4,4,2,endpoint=True)
sim.t = 0
for k, hour in enumerate(hours):
    ...
    fig.savefig("canwint/plot_%05d.png"%k, facecolor="white", dpi=200,bbox_inches='tight')
    plt.close(fig)
